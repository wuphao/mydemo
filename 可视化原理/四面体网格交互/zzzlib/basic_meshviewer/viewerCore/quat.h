...
public:
    ...
    */
    ...
    CQrot(double _theta, CPoint _axis)
    {
        _axis /= _axis.norm();

        m_w = cos(_theta * 0.5);
        _axis *= sin(_theta * 0.5);
    ...
    */
    CPoint rot(CPoint p)
    {
        CQrot pp(0, p[0], p[1], p[2]);
        normalize();
        CQrot qq(m_w, m_x, m_y, m_z);
        CQrot q(m_w, -1*m_x, -1*m_y, -1*m_z);
        CQrot re = qq * pp * q;
        return CPoint(re.m_x, re.m_y, re.m_z);
    }
