...
struct Complex {
	...
	Complex operator/(const Complex& c) {
		return Complex({ r,i })*c.conj() * (1.0 / (c.r*c.r + c.i * c.i));
	}
	
};

Complex QuaternionTransform(float x, float y) {
	Complex a = { operQuat.m_w,-operQuat.m_z }, b = { -operQuat.m_y,-operQuat.m_x };
	Complex z = { x,y };
	Complex fz = (a * z + b) / (-b.conj() * z + a.conj());
	return fz;
}
...
void draw_mesh()
{
	...
	for (int i = 0; i < precision; i++) {
		for (int j = 0; j < precision; j++) {
			...
			float sx = (float)i / precision, sy = (float)j / precision;

			float ex = sx + 1.0 / precision, ey = sy + 1.0 / precision;

			//Quaternion Transform

			Complex start = QuaternionTransform(sx, sy);
			Complex end = QuaternionTransform(ex, ey);

			glBegin(GL_POLYGON);

			glTexCoord2d(start.r, start.i);
			glVertex3f(sx, sy, 0);

			glTexCoord2d(end.r, start.i);
			glVertex3f(ex, sy, 0);

			glTexCoord2d(end.r, end.i);
			glVertex3f(ex, ey, 0);

			glTexCoord2d(start.r, end.i);
			glVertex3f(sx, ey, 0);
